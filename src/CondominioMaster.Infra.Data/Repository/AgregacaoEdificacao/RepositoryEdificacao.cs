...
namespace CondominioMaster.Infra.Data.Repository.AgregacaoEdificacao
{
     ...
     public class RepositoryEdificacao : Repository<Edificacao>, Domain.Interfaces.Repository.AgregacaoEdificacao.RepositoryEdificacao
     {
          ...
          */
          ...
          public Edificacao ObterEdificacaoPorID(int idEdificacao)
          {
               //  através do Entity
               //   return Db.Empresa.AsNoTracking().FirstOrDefault(e => e.CPFCNPJ.Numero == cpfcnpj);
               /* Usando o Dapper, melhor performace */
               ...
               var retorno = Db.Database.GetDbConnection().Query<Edificacao>(query.ToString(), new { uID = idEdificacao }).FirstOrDefault();
               return retorno;
          }
